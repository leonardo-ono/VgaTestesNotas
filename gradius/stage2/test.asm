...
start:
	...
	.next_sprite_restore:
		...
		mov ax, [brick_x]
		mov word [restore_stage.x], ax ; 201
		mov ax, [brick_y]
		mov word [restore_stage.y], ax ; 122
		mov word [restore_stage.width], 16
		mov word [restore_stage.height], 16
		call restore_stage

		pop cx
		loop .next_sprite_restore
...
restore_stage:
		...
		; and word [.y], 0fffch ; fix .y for 8 bytes align <--- nao funcionou
		mov ax, [.y] ; fix .y for 8 bytes align
		mov cl, 3
		shr ax, cl
		shl ax, cl
		mov [.y], ax
